...
export default function Footer() {
  ...
  return (
    <>
      <footer className="flex flex-col h-auto  items-center justify-around px-6 py-20 md:flex-row bg-main-color">
        <Image
          src={mypic}
          width={220}
          height={220}
        ...
        <div className="w-full h-full text-center sm:text-left sm:w-8/12  grid grid-cols-1 gap-7">
          ...
          <form className="flex">
            <input
              ...
              value={email.email}
              className="w-7/12 h-12 rounded-l-xl p-3"
              placeholder="Email"
            />
            <button
              onClick={submitSubscription}
              type="submit"
              className="w-5/12 h-12 hidden md:block bg-yellow-500 rounded-r-xl tracking-wide font-semibold"
            >
            ...
            <button
              onClick={submitSubscription}
              type="submit"
              className="w-5/12 h-12 md:hidden bg-yellow-500 rounded-r-xl tracking-wide font-semibold"
            >
              Subscribe
            </button>
          </form>
          {isLoading && (
            <center className="text-center font-bold text-lg">Loading</center>
          )}
          {errorMessage && (
